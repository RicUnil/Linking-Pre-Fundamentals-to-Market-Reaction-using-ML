"""
Experiment 02: 5-Day Excess Returns Prediction

This experiment tests whether very short-term post-earnings returns (5-day)
are more predictable than longer horizons (10-day, 30-day).

Research Question: Does market efficiency hold at 5-day horizon?

Key principles:
- Reuses cleaned features from Step 10 (no modification of main pipeline)
- Computes new 5-day excess return target
- Trains lightweight models: Baseline, Ridge, Random Forest
- Saves outputs to experiments_02/results/returns_5d/

Usage:
    python3 experiments_02/src/experiment_02_returns_5d.py
"""
...
def setup_logging() -> logging.Logger:
    ...
    logger = logging.getLogger("experiment_02_returns_5d")
...
def build_5d_target(
    ...
    """
    Compute 5-day excess return for each earnings event.
    
    excess_return_5d = (stock return day 1→5) - (SPY return day 1→5)
    ...
    -------
    pd.Series
        5-day excess returns aligned with df index
    """
    logger.info("\nComputing 5-day excess returns...")
    ...
    # Compute 5-day returns for each event
    excess_returns = []
    ...
    for idx, row in df.iterrows():
        ...
        # Define window: day 1 to day 5 after earnings
    ...
    result = pd.Series(excess_returns, index=df.index, name='excess_return_5d')
    
    # Log statistics
    valid_count = result.notna().sum()
    logger.info(f"  ✓ Computed {valid_count:,} / {len(result):,} valid 5-day returns")
...
def align_targets(
    ...
    """
    Build 5-day targets for train/val/test splits and remove NaN rows.
    ...
    Returns
    -------
    Tuple of (y_train_5d, y_val_5d, y_test_5d, updated data dict)
    """
    logger.info("=" * 70)
    logger.info("BUILDING 5-DAY TARGETS")
    logger.info("=" * 70)
    
    daily_data_dir = settings.get_step_results_dir(5)
    
    # Build targets for each split
    y_train_5d = build_5d_target(data['cleaned_train'], daily_data_dir, logger)
    y_val_5d = build_5d_target(data['cleaned_val'], daily_data_dir, logger)
    y_test_5d = build_5d_target(data['cleaned_test'], daily_data_dir, logger)
    ...
    y_train_5d = y_train_5d[train_valid].values
    logger.info(f"  New train size: {len(y_train_5d):,}")
    
    # Validation
    val_valid = y_val_5d.notna()
    logger.info(f"\nValidation: Removing {(~val_valid).sum():,} NaN rows")
    data['X_val'] = data['X_val'][val_valid]
    data['cleaned_val'] = data['cleaned_val'][val_valid]
    y_val_5d = y_val_5d[val_valid].values
    logger.info(f"  New val size: {len(y_val_5d):,}")
    
    # Test
    test_valid = y_test_5d.notna()
    logger.info(f"\nTest: Removing {(~test_valid).sum():,} NaN rows")
    data['X_test'] = data['X_test'][test_valid]
    data['cleaned_test'] = data['cleaned_test'][test_valid]
    y_test_5d = y_test_5d[test_valid].values
    logger.info(f"  New test size: {len(y_test_5d):,}")
    ...
    return y_train_5d, y_val_5d, y_test_5d, data
...
def save_outputs(
    ...
    output_dir = PROJECT_ROOT / "experiments_02" / "results" / "returns_5d"
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Save targets
    logger.info("\nSaving targets...")
    np.save(output_dir / "y_train_5d.npy", y_train)
    np.save(output_dir / "y_val_5d.npy", y_val)
    np.save(output_dir / "y_test_5d.npy", y_test)
    logger.info("  ✓ Targets saved")
    
    # Save metrics as JSON
    logger.info("\nSaving metrics (JSON)...")
    import json
    with open(output_dir / "regression_metrics_5d.json", 'w') as f:
    ...
    metrics_df.to_csv(output_dir / "regression_metrics_5d.csv")
    logger.info("  ✓ Metrics CSV saved")
    
    # Save predictions
    logger.info("\nSaving predictions...")
    for pred_name, pred_values in predictions.items():
        np.save(output_dir / f"predictions_5d_{pred_name}.npy", pred_values)
    ...
    residuals_df.to_parquet(output_dir / "residuals_5d_ridge.parquet")
    logger.info("  ✓ Residuals saved")
    
    # Save completion marker
    logger.info("\nSaving completion marker...")
    with open(output_dir / "step_experiment_02_completed.txt", 'w') as f:
        f.write("Experiment 02: 5-day excess returns completed successfully.\n")
...
def main():
    ...
    logger.info("\n" + "=" * 70)
    logger.info("EXPERIMENT 02: 5-DAY EXCESS RETURNS")
    logger.info("=" * 70)
    logger.info("\nObjective: Test if 5-day returns are more predictable than longer horizons")
    logger.info("Strategy: Reuse Step 10 features, compute new 5-day target")
    logger.info("=" * 70)
    ...
    try:
        ...
        # Step 2: Build and align 5-day targets
        y_train_5d, y_val_5d, y_test_5d, data = align_targets(data, logger)
        
        # Step 3: Train models
        models = train_models(
            data['X_train'], y_train_5d,
            data['X_val'], y_val_5d,
            logger
        )
        ...
        # Final summary
        logger.info("\n" + "=" * 70)
        logger.info("EXPERIMENT 02 COMPLETED SUCCESSFULLY")
        logger.info("=" * 70)
        logger.info("\nTest Set Results:")
